...
internal class NativeListTests : CollectionsTestFixture
{
    ...
    [Test]
    public void NativeList_CountAndLength_AreEqual()
    {
        var list = new NativeList<int>(Allocator.Persistent);
        list.Length = 3;
        Assume.That(list.Length, Is.EqualTo(3));
        Assert.That(list.Length, Is.EqualTo(list.Count));

        list.Count = 5;
        Assume.That(list.Length, Is.EqualTo(5));
        Assert.That(list.Length, Is.EqualTo(list.Count));
    ...
    public void NativeList_CopyFrom_OtherContainers()
    {
...

        {
            ...
            list.CopyFrom(container.AsReadOnlySpan());
            ExpectedLength(ref list, 8);
            for (int i = 0; i < list.Length; ++i)
            {
                Assert.AreEqual(i, list[i]);
    ...
    static IEnumerable<TestCaseData> AddRangeData()
    {
        var list = new NativeList<int>(Allocator.Persistent);
        for (var i = 0; i < 10; ++i)
            list.Add(i);
        yield return new TestCaseData(list)
            { TestName = nameof(NativeList<int>) };
    }

    [Test]
    [TestCaseSource(nameof(AddRangeData))]
    public void NativeList_AddRange(NativeList<int> testData)
    {
        for (var i = 0; i < testData.Length; ++i)
            Assume.That(testData[i], Is.EqualTo(i));
        
        using var list = new NativeList<int>(Allocator.Persistent);
        list.AddRange(testData);
        
        Assert.That(list.Length, Is.EqualTo(testData.Length));
        for (var i = 0; i < list.Length; ++i)
            Assert.That(list[i], Is.EqualTo(i));

        if (testData is INativeDisposable disposable)
            disposable.Dispose();
    }
}
