...
internal class NativeParallelHashSetTests: CollectionsTestFixture
{
    ...
    public void NativeParallelHashSet_IsEmpty()
    {
        ...
        Assert.IsTrue(container.TryAdd(0));
    ...
    public void NativeParallelHashSet_Collisions()
    {
        ...
        for (int i = 0; i < 8; ++i)
        {
            Assert.IsTrue(container.TryAdd(i), "Failed to add value");
        }
        ...
        for (int i = 0; i < 8; ++i)
        {
            Assert.IsTrue(container.TryAdd(i + 32), "Failed to add value with potential hash collision");
        }
    ...
    [Test]
    public void NativeParallelHashSet_SameKey()
    {
        using var container = new NativeParallelHashSet<int>(4, Allocator.Persistent);
        Assert.DoesNotThrow(() => container.Add(0));
        Assert.IsFalse(container.Add(0)); // Nuance: This version of Add does not throw (but will eventually)!
        Assert.IsFalse(container.TryAdd(0));
        Assert.IsTrue(container.Contains(0));
    }

    [Test]
    public void NativeParallelHashSet_ParallelWriter_SameKey()
    {
        using var container = new NativeParallelHashSet<int>(4, Allocator.Persistent);
        var writer = container.AsParallelWriter();
        Assert.DoesNotThrow(() => writer.Add(0));
        Assert.IsFalse(writer.Add(0)); // Nuance: This version of Add does not throw (but will eventually)!
        Assert.IsFalse(writer.TryAdd(0));
        Assert.IsTrue(container.Contains(0));
    }
