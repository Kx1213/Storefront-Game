...
#pragma warning restore 0649
    ...
    [Test]
    public void NativeParallelHashMap_SameKey()
    {
        using var container = new NativeParallelHashMap<int, int>(4, Allocator.Persistent);
    ...
    [Test]
    public void NativeParallelHashMap_ParallelWriter_SameKey()
    {
        using var container = new NativeParallelHashMap<int, int>(4, Allocator.Persistent);
        var writer = container.AsParallelWriter();
        Assert.DoesNotThrow(() => writer.Add(0, 5));
#if ENABLE_UNITY_COLLECTIONS_CHECKS || UNITY_DOTS_DEBUG
        Assert.Throws<ArgumentException>(() => writer.Add(0, 6));
#else
        Assert.DoesNotThrow(() => writer.Add(0, 6)); // Ensures the write gracefully fails when collections checks are disabled.
#endif
        Assert.IsFalse(writer.TryAdd(0, 7));
        Assert.AreEqual(container[0], 5);
    }

    [Test]
    public void NativeParallelHashMap_Resize_Via_Add_Works()
    {
