...
![None of the nodes are missing operands, so none are orange](images/vs-debug-nodes-correct.png)
...
![The owner GameObject doesn't have a rigidbody, which is a required component, so the node is yellow.](images/vs-debug-missing-component.png)
...
![In Play mode, the faulty node is highlighted in red](images/vs-debug-faulty-node-highlighted.png)
