...
* **Performance degradation**: The `TempJob` allocator has a fixed memory pool. If long-lived allocations accumulate and exhaust this pool, subsequent allocations fall back to `Allocator.Persistent`, which is significantly slower.
* **Memory leak reports**: If allocations still exist when the `TempJob` allocator is disposed (for example, when the application exits), Unity reports them as memory leaks when [native leak detection](https://docs.unity3d.com/ScriptReference/Unity.Collections.NativeLeakDetection.html) is enabled.
...
If you need an intermediate-lifetime allocator optimized for durations between `TempJob` and `Persistent`, consider using a [custom allocator](allocator-custom-define.md) or a [rewindable allocator](allocator-rewindable.md).
